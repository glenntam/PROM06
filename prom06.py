...
def get_bot_response():
    ...
    if request.method == 'POST':  # user submits feedback survey
        ...
        feedback = request.form.get('feedbackBox')
        corpus = str(session['corpus'])
        insert_string = "INSERT INTO survey(q1, q2, q3, q4, q5, q6, q7, q8, q9, q10, feedback, corpus) VALUES (" \
                        + q1 + ", " + q2 + ", " + q3 + ", " + q4 + ", " + q5 + ", " + q6 + ", " + q7 + ", " \
                        + q8 + ", " + q9 + ", " + q10 + ", '" + feedback + "', '" + corpus + "');"
        conn = get_db_connection()
        conn.cursor().execute(insert_string)
        conn.commit()
        conn.close ()
        session.clear()
